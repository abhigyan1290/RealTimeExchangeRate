...
def get_supported_currency_pairs():
    # List of currencies that are supported in yFinance
    supported_currencies = ['USD', 'EUR', 'GBP', 'JPY', 'CHF', 'AUD', 'CAD', 'NZD']
...
@app.route('/')
def upload_form():
    currencies = get_supported_currency_pairs()
...
def currency_history():
    if request.method == 'POST':
        ...
        currency_pairs = get_supported_currency_pairs()
        periods = {
            '1 Day': '1d',
            '5 Days': '5d',
            '1 Month': '1mo',
        ...
        return render_template('history_form.html', currency_pairs=currency_pairs, periods=periods)
